...
function UndraggableTarget(){
  ...
  $('#m'+id+' .mobjhan').text("■");
  $('#m'+id+' .mobjhan').css("color","darkgrey");
}
//--------------------------------------------------------------------
  ...
  $.get('/pie/diaryall-at/'+mobjid,
    ...
    function( data ){
      ...
      $.post('/momo/update/'+mobjid, "momo[c_tag]=",
        function(){ location.href = '/pie/diaryall-at/'+mobjid }
      );
    }
  );
  origtext = "";
...
function iPE_Pic( target ) {
    ...
    '<input type="hidden" name="author" value="pie"/>'+
    '<input type="hidden" name="momo[c_author]" value="pie"/>'+
    '<input type="hidden" name="page" value="diary-new"/>'+
...
function insertPicAjax( si ){
  ...
  $.get('/pie/diaryall-at/'+mobjid,
    ...
    function( data ){
      ...
      location.href = '/pie/diaryall-at/'+mobjid;
    }
  );
}
function insertPic(){
...
function startInsMenu( e,id ){
  if( $('#Inscontextmenu').css('display')!='block' ){
    ...
    $('#Inscontextmenu').css('z-index','100' );
    $('#MobjId').text( id );
...
function editMobj(){
 location.href='/pie/diary-edit/'+$('#MobjId').text()
}

function insertMobj(){
 location.href='/pie/diary-insert/'+$('#MobjId').text()
}
...
function cutMobj(){
  ...
  $.get('/pie/diaryall-at/'+mobjid,
    {'m_subfix':'yes','m_tr':'no'},
    function( data ){
      $.post('/momo/update/'+mobjid,
         'author=pie&'+
         'page=diary-new&'+
         'momo[c_author]=pie&'+
         'momo[c_tag]='+getIdVal( data, mobjid, 'c_tag' ).replace('[diary]','[cut]'),
        function(){ location.href = '/pie/diaryall-at/'+mobjid }
      );
    }
  );
...
function deleteMobj( fromall ){
  ...
  if( confirm('Are you sure? ['+mobjid+']') ) {
    $.get('/momo/destroy/'+mobjid ,{},
      function( data ){
        if( fromall ) {
          location.href = '/pie/diaryall';
        }else{
          location.href = '/pie/diaryall-at/'+top_id;
        }
      }
    );
  }
