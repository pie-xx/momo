...
<div class="mobjContainer">
  ...
  <table>
  <xsl:if test="not(contains(c_status,'[public]'))">
    <xsl:attribute name="style">background-color:lightgreen;</xsl:attribute>
  </xsl:if>
  <tr>
  <xsl:if test="/momo/prof/user=/momo/prof/author">
  ...
  <xsl:if test="not($id='')">
    <xsl:if test="/momo/prof/user=/momo/prof/author">
      <button style="font-size:8pt;">
        ...
        location.href='/pie/diary-edit?id=<xsl:value-of select="$id"/>'
        </xsl:attribute>
        e <xsl:value-of select="$id"/>
      </button>
    </xsl:if>
  </xsl:if>
</xsl:template>
...
<xsl:template name="insertbutton">
  ...
  <xsl:if test="not($id='')">
    <xsl:if test="/momo/prof/user=/momo/prof/author">
      <button style="font-size:8pt; background-color:lightblue;">
        <xsl:attribute name="onclick">
        location.href='/pie/diary-insert?id=<xsl:value-of select="$id"/>'
        </xsl:attribute>
...
<div id="contextmenu">
  ...
  <button class="menubutton" onMouseUp="editMobj();"><img src="/icons/32px-Crystal_Clear_action_configure.png"/><br/>config</button>
  ...
  <button class="menubutton" onMouseUp="insertMobj();"><img src="/icons/32add_any.png"/><br/>+insert</button>
</div>
...
<!-- ////////////////////////////////////////////////////////////////////// -->
<xsl:template name="TrackBackForm">
<input type="text" id="trackback" size="80"/>
<button id="tbbutton">
  <xsl:attribute name="onclick">
    var tbbutton = document.createElement('tbbutton');
    tbbutton.disabled = true;
    var f = document.createElement('form');
    f.style.display = 'none';
    this.parentNode.appendChild(f);
    f.method = 'POST';
    f.action = document.getElementById('trackback').value;
    var u = document.createElement('input');
    u.setAttribute('type', 'text');
    u.setAttribute('name', 'url');
    u.setAttribute('value', 'http://m-obj.com/pie/diary-at/<xsl:value-of select="/momo/prof/id"/>');
    f.appendChild(u);
    var b = document.createElement('input');
    b.setAttribute('type', 'text');
    b.setAttribute('name', 'blog_name');
    b.setAttribute('value', "pie's diary");
    f.appendChild(b);
    var t = document.createElement('input');
    t.setAttribute('type', 'text');
    t.setAttribute('name', 'title');
    t.setAttribute('value', '<xsl:value-of select="/momo/item0/item1/rec/c_index"/>');
    f.appendChild(t);
    f.submit();
    var e = document.createElement('input');
    e.setAttribute('type', 'text');
    e.setAttribute('name', 'excerpt');
    e.setAttribute('value', '<xsl:value-of select="substring(/momo/item0/item1/rec/c_body,0,20)"/>');
    f.appendChild(e);
    f.submit();
